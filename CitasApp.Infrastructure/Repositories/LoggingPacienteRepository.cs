...
namespace CitasApp.Infrastructure.Repositories
{
    ...
    public class LoggingPacienteRepository : IPacienteRepository
    {
        ...
        {
            Console.WriteLine($"[{DateTime.Now:yyyy-MM-dd HH:mm:ss}] Agregar({paciente.Id})");
            _inner.Agregar(paciente);
        }

        public void Actualizar(Paciente paciente)
        {
            Console.WriteLine($"[{DateTime.Now:yyyy-MM-dd HH:mm:ss}] Actualizar({paciente.Id})");
            _inner.Actualizar(paciente);
        }

        public void Eliminar(int id)
        {
            Console.WriteLine($"[{DateTime.Now:yyyy-MM-dd HH:mm:ss}] Eliminar({id})");
            _inner.Eliminar(id);
        }
    }
}